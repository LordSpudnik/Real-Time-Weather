...
async function checkWeather(city) {
  ...
  try {
    ...
    document.querySelector(".form-container").style.backgroundColor =
      "rgba(54, 47, 47, 0.8)";
    document.querySelector(".favorites-container").style.backgroundColor =
      "rgba(54, 47, 47, 0.8)";
    ...
    predElem.innerHTML = `
            ...
            <p><strong>${futureTimes[0]}</strong>: ${Math.round(
      pred.predicted_temperature_3h
    )}°C</p>
            <p><strong>${futureTimes[1]}</strong>: ${Math.round(
      pred.predicted_temperature_6h
    )}°C</p>
            <p><strong>${futureTimes[2]}</strong>: ${Math.round(
      pred.predicted_temperature_9h
    )}°C</p>
        `;
...
function resetUI() {
  ...
  document.querySelector(".weather-container").style.background =
    "rgb(255, 255, 255, 0.2)";
  document.querySelector(".form-container").style.background =
    "rgb(255, 255, 255, 0.2)";
  document.querySelector(".favorites-container").style.background =
    "rgb(255, 255, 255, 0.2)";
...
async function submitPhoneNumber() {
  ...
  document.querySelector(".weather-container").style.transform =
    "translateX(-100%)";
    ...
    try {
      ...
      if (response.status === 409) {
        const userConfirmed = confirm(
          "Alert already exists for this phone number. Do you want to update it?"
        );
        if (userConfirmed) {
...
async function removeCity(city) {
  ...
  const cityItem = Array.from(favoritesList.children).find((item) =>
    item.textContent.includes(city)
  );
  if (cityItem) {
